...
<div class="card ml-5 mr-5 mt-2">
    ...
    <div class="card-header" style="background-color:#558b2f ">
        <div class="d-flex justify-content-center text-white">
            <h2>EDITAR SOLICITUD DE ANÁLISIS</h2>
        </div>
    </div>
    ...
    <div class="card-body px-5" style="background-color: #e6e6e6; padding-bottom: 56px;">
        ...
        <form action="{{ route('admin.solicitud_analisis.update', $solicitud->id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-6">
                    {{-- <div class="form-group">
                        ...
                        <div class="input-container">
                            <input type="text" id="cliente" name="cliente" class="form-control" autocomplete="off" placeholder="Buscar Nombre del cliente..." value="{{ $solicitud->cliente->persona->nombre }} {{ $solicitud->cliente->persona->apellido }} - {{ $solicitud->cliente->propiedad ? $solicitud->cliente->propiedad->nombre : 'Sin propiedad' }}">
                    ...
                    <div class="form-group">
                        <label for="cliente">Buscar Nombrek del Cliente:</label>
                        <div class="input-container">
                            <input type="text" id="cliente" name="cliente" class="form-control" autocomplete="off"
                                placeholder="Buscar Nombre del cliente..."
                                value="{{ $solicitud->cliente->persona->nombre }} {{ $solicitud->cliente->persona->apellido }} - {{ $solicitud->cliente->propiedad ? $solicitud->cliente->propiedad->nombre : 'sin propiedad' }}">
                            <i class="fas fa-search"></i>
                            <div id="resultados_cliente" class="list-group" style="position: absolute; z-index: 1000; background: white; width: 100%; display: none;"></div>
                        </div>
                        @error('cliente_id')
                            <p style="color: red; font-size: 1.0em;">{{ $message }}</p>
                        @enderror
                        <input type="hidden" id="cliente_id" name="cliente_id" value="{{ $solicitud->cliente_id }}">
                    </div>
                </div>
                ...
                <div class="col-md-3">
                    ...
                    <select class="form-control" name="gestion_id" id="gestion_id" style="width: 100%;">
                        <option value="" selected>Seleccione una gestión...</option>
                        @foreach ($gestiones as $gestion)
                            <option value="{{ $gestion->id }}" {{ old('gestion_id', $solicitud->gestion_id) == $gestion->id ? 'selected' : '' }}>
                                {{ $gestion->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-3">
                    <div class="form-group">
                        ...
                        <input type="date" name="fecha_muestreo" id="fecha_muestreo" class="form-control" value="{{ old('fecha_muestreo', $solicitud->fecha_muestreo) }}">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <div class="input-container">
                            <textarea name="glosario" id="glosario" cols="50" rows="2" class="form-control capitalize-first-word" placeholder="Ingrese descripción de la solicitud a realizar...">{{ old('glosario', $solicitud->glosario) }}</textarea>
                            @error('glosario')
                                <p style="color: red; font-size: 1.0em;">{{ $message }}</p>
                            @enderror
                        </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="lugar_muestreo" class="form-control capitalize-first-word" placeholder="Describa la Zona del terreno de donde se obtuvo la muestra..." value="{{ old('lugar_muestreo', $solicitud->lugar_muestreo) }}">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="cultivo_anterior" class="form-control capitalize-first-word" placeholder="Ingrese el cultivo anterior realizado..." value="{{ old('cultivo_anterior', $solicitud->cultivo_anterior) }}">
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <input type="text" name="cultivo_actual" class="form-control capitalize-first-word" placeholder="Ingrese el cultivo actual realizado..." value="{{ old('cultivo_actual', $solicitud->cultivo_actual) }}">
            ...
            <div class="form-group" style="margin-top:30px;" >
                <button type="submit" class="btn btn-primary w-25 float-right">Actualizar
                </button>
            </div>
        </form>
    </div>
...
//                             item.textContent = `${cliente.persona.nombre} ${cliente.persona.apellido} - ${cliente.propiedad.nombre}`;
//                             item.addEventListener('click', function(e) {
//                                 e.preventDefault();
//                                 document.getElementById('cliente').value = `${cliente.persona.nombre} ${cliente.persona.apellido} - ${cliente.propiedad.nombre}`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    queryInput.addEventListener('keyup', function() {
        ...
        if (query.length > 2) {
            fetch(`/admin/buscar-cliente?query=${query}`)
                .then(response => response.json())
                .then(data => {
                    resultados.innerHTML = ''; // Limpiar resultados anteriores
    ...
    // Ocultar resultados al hacer clic fuera del input o de la lista de resultados
    document.addEventListener('click', function(e) {
        if (!resultados.contains(e.target) && e.target !== queryInput) {
            resultados.style.display = 'none';
            resultados.innerHTML = '';
        }
    });
